///get_nearest_instances(object, closestto, maxnum); //Returns the MAXNUM closest instances as a ds_stack.
//REMEMBER to destroy the stack after using it!
//THIS FUNCTION IS QUITE RESOURCE INTENSIVE WITH A LOT OF INSTANCES.
//Example:

/*
//Find the 8 nearest instances and draw a sprite there
nearest = get_nearest_instances(obj_find_this, id, 8);
while (!ds_stack_empty(nearest)) {
    inst = ds_stack_pop(nearest);
    draw_sprite(spr_this_one_is_near, 0, inst.x, inst.y);
}
